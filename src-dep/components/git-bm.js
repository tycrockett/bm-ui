...
export const GitBm = ({ pwd }) => {
  ...
  const [hasRemote] = useAsyncValue(
    () => hasRemoteBranch(branches?.current || ""),
    [pwd, branches?.current, lastCommand]
  );
  ...
  useEffect(() => {
    if (lastCommand.endsWith("status")) {
      ...
      setTab("logs");
    } else if (lastCommand.endsWith("toggle-branches-dropdown")) {
      setOpenBranches(!openBranches);
    } else if (lastCommand.endsWith("hide-branches-dropdown")) {
  ...
  const handleDiff = async (path) => {
    ...
    for (let idx = 0; idx < diffs.length; idx++) {
      const hasChange =
        diffs[idx].startsWith("+") || diffs[idx].startsWith("-");
  ...
  const lineChanges = useMemo(() => {
    ...
    const { adds, deletes } = files.reduce(
      (prev, item) => {
        return {
          adds: prev.adds + Number(item.adds),
          deletes: prev.deletes + Number(item.deletes),
        };
      },
      { adds: 0, deletes: 0 }
    );
    const fileCount = Object.values(status?.fileCount || {}).reduce(
      (p, v) => p + Number(v),
      0
    );
    return {
    ...
    <Div styles="relative">
      ...
      >
        <Div styles="flex mt padb">
          <Button
            styles={tab === "status" ? "" : "hover text radius"}
            onClick={() => setTab("status")}
          >
            Status
          </Button>
          <Button
            styles={tab === "logs" ? "" : "text hover radius"}
            onClick={() => setTab("logs")}
          >
            Logs
          </Button>
        ...
        {tab === "logs" && (
          <Div styles="mt">
            {!Object.keys(logs || {})?.length ? (
              <Text styles="ml-sm">No commit logs.</Text>
            ) : (
                ...
                return (
                  <Div key={key} styles="pad-sm oa">
                    <Div styles="ai:c mb-xs">
                      ...
                      <Text as="span" styles="ml-sm">
                        {title}
                      </Text>
                    </Div>
                    {values?.map((file) => (
                      <Div styles="jc:sb ai:c hover radius pointer">
                        <Text key={file} styles="pad-xs padl">
                          {file}
                        </Text>
                        <Div styles="flex">
            ...
            <Div styles="mt">
              ...
              !status?.modified?.length ? (
                ...
                <Div styles="jc:sb ai:c mb">
                  <Text styles="ml-sm">
                    Updated at {format(new Date(status?.lastUpdate), "h:mm a")}
                  </Text>
                  <Div styles="flex right">
                    <Text styles="ml-sm" color={colors.lightGreen}>
                      +{lineChanges?.adds}
                    </Text>
                    <Text
                      className={css`
                ...
                {!!status?.untracked?.length && (
                  <Div styles="radius padv-sm">
                    <Div styles="ai:c">
                      ...
                      <Text styles="padh-xs bold" color={colors.lightBlue}>
                        Untracked
                      </Text>
                    </Div>
                        ...
                        >
                          ...
                          >
                            <Text color={colors.lightBlue}>
                              +{status?.fileCount?.[item]}
                            </Text>
                            <Button styles="icon-dark">
                              <TrashSimple color="white" size={24} />
                            </Button>
                          </Div>
                        </Div>
                      ))}
                ...
                {!!status?.deleted?.length && (
                  <Div styles="radius padv-sm">
                    <Div styles="ai:c">
                      ...
                      <Text styles="padh-xs bold" color={colors.lightRed}>
                        Deleted
                      </Text>
                    </Div>
                    <Div styles="flex wrap radius">
                ...
                {!!status?.modified?.length && (
                  <Div styles="radius padv-sm">
                    <Div styles="ai:c">
                      ...
                      <Text styles="padh-xs bold" color={colors.lightGreen}>
                        Modified
                      </Text>
                    </Div>
                    <Div styles="flex wrap radius">
                  ...
                  <pre
                    ...
                    className={css`
                      ...
                      ${line.startsWith(">>>>> Lines: ")
                        ? `color: black; background-color: ${colors.lightBlue};`
                        : line.startsWith("+")
